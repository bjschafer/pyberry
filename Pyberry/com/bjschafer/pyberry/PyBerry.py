...
def search_book_helper():
    ...
    search_field = eval(input("Which would you like to search by? "))
    search_term = eval(input("OK, go ahead: "))
...
def add_book():
    add_option = eval(input('''Let's add a book.  How would you like to add it?\n
                              1) Manually\n
                              2) Search by ISBN\n
                              3) Search by Title: '''))
    ...
    if add_option == 1:
        ...
        isbn = eval(input("Please enter the 10 or 13 digit ISBN: "))
        lookup = Lookup()
...
def edit_book():
    to_do = eval(input("We're going to edit a book.  Do you have the barcode? [y/N]: "))
    if to_do.strip() == "" or to_do.strip().lower() == "n":
        results = search_book_helper()
        i = 1
        for item in results:
            print((str(i) + ") " + str(item)))  # not sure how this will come out, same as above.
        edit_choice = eval(input("Which one is it? "))
        ...
        edit_choice = eval(input("Ok, enter it now: "))
...
def change_db_location(loc):
    ...
    change_it = eval(input("Would you like to change it? [y/N]: "))

    if change_it.strip().lower() != 'y':
        return
    else:
        new_loc = eval(input('''Please enter a new path, either relative to current directory\n
...
def add_person():
    ...
    print()
    first_name = input("What's the person's first name?")
    ...
    uid = input("If yes, enter it, otherwise enter -1")
...
def lend_book():
    ...
    todo = input("Do you have their unique ID? [y/N]: ")
    if todo.strip() == "" or todo.strip().lower() == 'n':
        ...
        to_do = input("Do you have its barcode? [y/N]: ")
        if to_do.strip() == "" or to_do.strip().lower() == 'n':
            results = search_book_helper()
            for item in results:
                print(str(i) + ") " + str(item))
            book_lend = input("Which one is it? ")
            ...
            user_date = input("When is it due? [" + str(due_date) + "]: ")
            if user_date.strip() == "":
            ...
            if bc.strip() == '':
                ...
                due_date = datetime.date.today() + datetime.timedelta(days=30)
                user_date = input("When is it due? [" + str(due_date) + "]: ") # assumptions
                if user_date.strip() == "":
                    loan = Loan(-1, book_lend, lend_person, datetime.date.today(), due_date, False)
                    the_db.store_loan(loan)
                else:
        ...
        if uid.strip() == '':
            ...
            lend_person = create_person_from_list(lend_person)
            print("Which book would you like to lend?")
            todo = input("Do you have its barcode? [y/N]: ")
            if todo.strip() == "" or todo.strip().lower() == 'n':
                results = search_book_helper()
                for i, item in enumerate(results):
                    print(str(i) + ") " + str(item))
                ...
                if bc.strip() == '':
                    ...
                    due_date = datetime.date.today() + datetime.timedelta(days=30)
                    user_date = input("When is it due? [" + str(due_date) + "]: ") # just going to assume it's in YYYY-MM-DD form for now.
                    if user_date.strip() == "":
...
def debug_menu():
    ...
    dbg = eval(input('''1) Config file location'''))
    try:
        dbg = int(dbg)
    except ValueError:
        return
...
if __name__ == '__main__':
    ...
    while run:
        ...
        if todo == 1:
            ...
            todo = input('''Here's what you can do: \n
